...
const connectWithRetry = async () => {
  ...
  try {
    ...
    // FIX: Changed 8080 to 5000 (milliseconds)
...
app.use(
  session({
    ...
    cookie: {
      // FIX: Set to true for production HTTPS
...
app.put("/api/artisan/:id", upload.any(), async (req, res) => {
  const { id } = req.params;
  try {
    const {
    ...
    if (req.files && req.files.length > 0) {
      req.files.forEach((file) => {
        ...
        else if (file.fieldname === "portfolio") portfolioArray.push(file.filename);
      });
    }
    ...
    const query = `
      UPDATE artisans 
      SET 
        firstname = $1, lastname = $2, phone = $3, email = $4, 
        gender = $5, dob = $6, city = $7, address = $8, 
        skill = $9, experience = $10, bio = $11, 
    ...
    const values = [
      firstname, lastname, phone, email, gender, dob, city, 
      address, skill, experience, bio, profilePic, certificate, 
      reference, JSON.stringify(portfolioArray), parseInt(id)
    ];
...
messagesRouter.post("/", async (req, res) => {
  ...
  if (!sender_id || !receiver_id || receiver_id === "null" || !content?.trim()) {
    return res.status(400).json({ error: "Invalid message data or missing receiver" });
  }
  ...
  try {
    ...
    const artisanId = userResult.rows[0]?.artisanid;

    if (artisanId) {
      const artisanResult = await pool.query(
        "SELECT id, coins FROM artisans WHERE id = $1",
        [artisanId]
      );
      ...
      if (artisan) {
        const threadCheck = await pool.query(
          "SELECT COUNT(*) FROM messages WHERE receiver_id = $1 AND sender_id = $2",
          [parseInt(receiver_id), parseInt(sender_id)]
        );

        if (parseInt(threadCheck.rows[0].count) === 0) {
          if (artisan.coins < 25) {
            return res.status(403).json({ error: "Artisan leads full (Insufficient Coins)." });
          }
          await pool.query("UPDATE artisans SET coins = coins - 25 WHERE id = $1", [artisan.id]);
        }
      }
    }

    const newMessage = await pool.query(
      "INSERT INTO messages (sender_id, receiver_id, content, read, timestamp) VALUES ($1, $2, $3, false, NOW()) RETURNING *",
...
messagesRouter.get("/conversations-summary/:userId", async (req, res) => {
  ...
  if (!userId || userId === "null") return res.status(400).json([]);
  try {
    const result = await pool.query(
...
messagesRouter.get("/conversations/:userId/:otherUserId", async (req, res) => {
  ...
  if (!userId || userId === "null" || !otherUserId || otherUserId === "null") {
...
app.post("/api/register-artisan", upload.any(), async (req, res) => {
  const {
    firstname, lastname, phone, gender, dob, city, address,
    skill, experience, bio, reference, email, userId
  } = req.body;
  ...
  try {
    ...
    if (!firstname || !lastname || !phone || !email || !skill) {
    ...
    const result = await pool.query(
      `INSERT INTO artisans 
       ...
       RETURNING *`,
      [firstname, lastname, phone, gender, dob, city, address, skill, experience, bio, profilePic, certificate, reference, email, portfolio.length > 0 ? JSON.stringify(portfolio) : null]
    );
    ...
    if (userId) {
      ...
      if (req.session) {
        req.session.artisanId = artisan.id;
...
app.get("/api/artisan/:id/reviews", async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query(
...
app.post("/api/artisan/:id/add-job-posting", upload.single("image"), async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    res.status(200).json({ message: "Job posting added", jobPosting: result.rows[0] });
  } catch (err) {
    res.status(500).json({ error: "Server error" });
  }
});

app.get("/api/users/by-artisan/:artisanId", async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: "No user linked" });
    }
    res.status(200).json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: "Server error" });
...
app.put("/api/link-artisan-to-user", async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      req.session.userId = result.rows[0].id; 
      req.session.save(() => {
        res.status(200).json({ message: "Linked successfully", user: result.rows[0] });
      });
    } else {
...
app.get("/api/artisan/:id/coins", async (req, res) => {
  const { id } = req.params;
...
app.post("/api/artisan/:id/purchase-coins", async (req, res) => {
  const { id } = req.params;
...
app.post("/api/artisan/verify-payment", async (req, res) => {
...
app.post("/api/confirm-deal", async (req, res) => {
...
app.get("/api/auth/check-session", async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      // Map the lowercase DB column to the camelCase frontend expectation
      artisanId: user.artisanid, 
      role: user.role
    });
  } catch (err) {
...
app.patch("/api/artisan/:id/update-coins", async (req, res) => {
  const { id } = req.params;
