      ...
      <header className="relative h-[60vh] bg-stone-900 overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-full p-8 md:p-16">
          <div className="max-w-7xl mx-auto">
            <p className="text-amber-500 text-[10px] uppercase tracking-[0.4em] font-bold mb-4">Master of {artisan.skill}</p>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {artisan.portfolio.map((file, idx) => (
                    <div key={idx} className="overflow-hidden bg-stone-200 group relative">
                       {file.endsWith(".mp4") ? (
                         <video className="w-full h-full object-cover" muted loop autoPlay><source src={`http://localhost:8080/uploads/${file}`} /></video>
                       ) : (
                         <img src={`http://localhost:8080/uploads/${file}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-1000 grayscale-[20%] group-hover:grayscale-0" alt="Work" />
                       )}
                    </div>
                  ))}
               </div>
             ...
             <div className="space-y-6 mb-10">
                ...
                {artisan.certificate && (
                  <div className="flex justify-between border-b border-stone-50 pb-4">
                    ...
                    <a href={`http://localhost:8080/uploads/${artisan.certificate}`} target="_blank" className="text-amber-600 text-[10px] font-black uppercase hover:underline">View Proof</a>
                  </div>
                )}
             </div>
